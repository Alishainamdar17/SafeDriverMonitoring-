...
// // ─────────────────────────────────────────────────────────────────────────────
// //  SAFE DRIVE MONITOR — AI DRIVER MONITORING SYSTEM
// //  v2.2: Live Travel Mode (Drive History removed)
// // ─────────────────────────────────────────────────────────────────────────────
...
//       ..repeat(reverse: true);
//     _drowsyController = AnimationController(
...
//       if (isNight) _speak('Night mode activated. Drive carefully.');
//     }
//   }
...
//           frontCam, ResolutionPreset.low, enableAudio: false);
...
//       }
//     } catch (e) {
//       debugPrint('Camera init error: $e — using simulated detection');
//       _startSimulatedDrowsinessMonitoring();
//     }
//   }
...
//         _registerLongBlink(duration: 500 + Random().nextInt(300));
//       }
//     });
//   }
...
//   void _registerLongBlink({required int duration}) {
//     _longBlinkCount += 2;
//     _updateDrowsinessLevel();
//   }
...
//         case 3: _driverAttention = 'CRITICAL';    _attentionScore = Random().nextInt(30);      break;
//       }
//     });
//     if (newLevel > prevLevel) _onDrowsinessEscalated(newLevel);
//   }

//   void _onDrowsinessEscalated(int level) {
//     switch (level) {
//       case 1:
//         _speak('You seem a little tired. Stay alert.');
//         _deductSafetyScore(3);
//         break;
//       case 2:
//         _speak('Warning! Drowsiness detected. Please take a break.');
//         HapticFeedback.heavyImpact();
//         _deductSafetyScore(8);
//         break;
//       case 3:
//         _speak('CRITICAL! You are falling asleep! Pull over immediately!', force: true);
//         HapticFeedback.heavyImpact();
//         _deductSafetyScore(15);
//         break;
//     }
//   }

//   // ── Stop Detection & Trip Summary ──────────────────────────────────────────
//   void _checkStopDetection() {
//     if (_wasDriving && !_isDriving) {
//       _stoppedSeconds++;
//       if (_stoppedSeconds >= 30 && !_tripSummaryShown) _showTripSummary();
//     } else {
//       _stoppedSeconds = 0;
//     }
...
//     HapticFeedback.heavyImpact();
//     _speak('SOS activated. Calling emergency services in 5 seconds.', force: true);
...
//     _speak('SOS cancelled.', force: true);
//   }
...
//       final brakeCooldownOk = _lastBrakeTime == null ||
//           now.difference(_lastBrakeTime!).inMilliseconds > _eventCooldownMs;

//       if (mag > 4.5 && linearY < -3.5 && _currentSpeed > 10 && brakeCooldownOk) {
//         _lastBrakeTime = now;
//         setState(() => _hardBrakeCount++);
//         _deductSafetyScore(5);
//         HapticFeedback.heavyImpact();
//         _speak('Hard braking detected. Slow down smoothly.');
//       } else if (mag > 4.0 && linearY > 3.0 && _currentSpeed > 5) {
//         final accelCooldownOk = _lastAccelTime == null ||
...
//       final turnCooldownOk = _lastTurnTime == null ||
//           now.difference(_lastTurnTime!).inMilliseconds > _eventCooldownMs;

//       if (linearX.abs() > 3.5 && _currentSpeed > 15 && turnCooldownOk) {
//         _lastTurnTime = now;
//         setState(() => _sharpTurnCount++);
//         _deductSafetyScore(4);
//         HapticFeedback.mediumImpact();
//         _speak('Sharp turn detected. Slow down.');
//       }
//     });
//   }
...
//           _latitude, _longitude, position.latitude, position.longitude);
...
//       if (_isOverSpeed) {
//         _deductSafetyScore(0.5);
//         _overSpeedCount++;
//         HapticFeedback.mediumImpact();
//         if (!wasOverSpeed) _speak('Speed limit exceeded. Please reduce your speed.');
//       }
//     });
//   }

//   void _deductSafetyScore(double amount) {
//     setState(() => _safetyScore = (_safetyScore - amount).clamp(0.0, 100.0));
//   }
...
//                           const SizedBox(height: 14),
//                           // ── NEW: Live Trip Stats Card ──────────────────
//                           _buildLiveTripStatsCard(duration),
//                           const SizedBox(height: 14),
...
//   // ─────────────────────────────────────────────────────────────────────────
//   //  LIVE TRIP STATS CARD  (NEW — replaces Drive History)
//   // ─────────────────────────────────────────────────────────────────────────
//   Widget _buildLiveTripStatsCard(Duration duration) {
//     final isActive = _isDriving;
//     final pulseColor = isActive ? const Color(0xFF34C759) : const Color(0xFF636366);
...
//         border: Border.all(
//           color: pulseColor.withValues(alpha: 0.25), width: 1.5,
//         ),
//         boxShadow: [
//           BoxShadow(
//             color: pulseColor.withValues(alpha: 0.06),
...
//                 width: 3, height: 16,
//                 decoration: BoxDecoration(
//                   color: pulseColor, borderRadius: BorderRadius.circular(2),
//                 ),
//               ),
...
//                   opacity: isActive ? _pulseAnim.value : 0.4,
//                   child: child,
//                 ),
//                 child: Container(
...
//                     border: Border.all(
//                         color: pulseColor.withValues(alpha: 0.4), width: 1),
//                   ),
//                   child: Row(
...
//                         width: 6, height: 6,
//                         decoration: BoxDecoration(
//                           color: pulseColor, shape: BoxShape.circle,
//                         ),
//                       ),
//                       const SizedBox(width: 5),
//                       Text(
//                         isActive ? 'LIVE' : 'PARKED',
//                         style: TextStyle(
//                           color: pulseColor, fontSize: 9,
...
//           // ── Big Speed + Distance Row ────────────────────────────────────
//           Row(
//             children: [
//               Expanded(
//                 child: _liveStatBlock(
//                   label: 'CURRENT SPEED',
...
//           const SizedBox(height: 14),
//           Divider(color: const Color(0xFF1C1C2E), height: 1),
//           const SizedBox(height: 14),
...
//               _liveStatMini(
//                 label: 'MAX SPEED',
//                 value: '${_maxSpeed.toStringAsFixed(0)} km/h',
//                 color: const Color(0xFFFF9F0A),
//                 icon: Icons.keyboard_double_arrow_up_rounded,
//               ),
//               _liveStatMini(
//                 label: 'SAFETY',
//                 value: '${_safetyScore.toInt()}/100',
//                 color: _scoreColor,
//                 icon: Icons.shield_rounded,
//               ),
//               _liveStatMini(
//                 label: 'OVER SPD',
//                 value: '$_overSpeedCount ×',
//                 color: _overSpeedCount > 0
...
//           Row(
//             children: [
//               Icon(Icons.gps_fixed_rounded, color: _accentBlue, size: 14),
//               const SizedBox(width: 8),
//               Text(
//                 _latitude == 0.0
...
//                 style: TextStyle(
//                   color: _latitude == 0.0 ? Colors.grey[600] : Colors.grey[400],
//                   fontSize: 11,
...
//                     'TRACKING',
//                     style: TextStyle(
//                       color: const Color(0xFF34C759), fontSize: 8,
//                       fontWeight: FontWeight.w800, letterSpacing: 1.2,
//                     ),
//                   ),
//                 ),
...
//           colors: [
//             const Color(0xFF1A0A3A).withValues(alpha: 0.9),
//             const Color(0xFF0A0A28).withValues(alpha: 0.9),
//           ],
//         ),
//         borderRadius: BorderRadius.circular(14),
//         border: Border.all(
//             color: const Color(0xFF5E5CE6).withValues(alpha: 0.4), width: 1),
//       ),
//       child: Row(
//         children: [
...
//                 ),
//                 Text(
//                   'Display optimized for nighttime driving',
...
//       builder: (_, child) {
//         final opacity = _drowsinessLevel >= 2 ? _drowsyAnim.value : 1.0;
...
//         decoration: BoxDecoration(
//           color: _drowsyColor.withValues(alpha: 0.08),
//           borderRadius: BorderRadius.circular(16),
//           border: Border.all(
//               color: _drowsyColor.withValues(alpha: 0.4), width: 1.5),
//         ),
//         child: Row(
//           children: [
//             Container(
//               width: 44, height: 44,
//               decoration: BoxDecoration(
//                 color: _drowsyColor.withValues(alpha: 0.15),
...
//         border: Border(
//             bottom: BorderSide(
//                 color: _accentBlue.withValues(alpha: 0.15), width: 1)),
//       ),
//       child: Row(
//         children: [
//           Row(
...
//           borderRadius: BorderRadius.circular(14),
//           border: Border.all(
//               color: alertColor.withValues(alpha: 0.5), width: 1),
//         ),
//         child: Row(
//           children: [
//             Container(
...
//                 color: alertColor.withValues(alpha: 0.6), size: 20),
//           ],
//         ),
//       ),
//     );
...
//               if (v) _speak('Voice alerts enabled.', force: true);
//             },
...
//           animation: _sosAnim,
//           builder: (_, child) => Transform.scale(
//             scale: _drowsinessLevel >= 2 ? _sosAnim.value : 1.0,
//             child: child,
//           ),
//           child: Container(
//             width: 64, height: 64,
...
//                         boxShadow: [
//                           BoxShadow(
//                             color: _speedColor.withValues(alpha: 0.18),
...
//                       color: _speedColor,
//                       limitProgress:
//                           (AppConfig.speedLimit / (AppConfig.speedLimit * 1.5))
//                               .clamp(0.0, 1.0),
//                     ),
//                   ),
//                   Column(
...
//                         decoration: BoxDecoration(
//                           color: _speedColor.withValues(alpha: 0.12),
//                           borderRadius: BorderRadius.circular(20),
//                           border: Border.all(
//                               color: _speedColor.withValues(alpha: 0.35),
//                               width: 1),
//                         ),
//                         child: Text(
...
//                   children: [
//                     _buildAIChip(
//                         Icons.visibility_rounded, _driverAttention, _attentionColor),
//                     const SizedBox(width: 6),
//                     _buildAIChip(
...
//           title: 'DRIVE TIME', value: _formatDuration(duration), unit: '',
//         )),
//         const SizedBox(width: 10),
//         Expanded(child: _buildMetricCard(
...
//         borderRadius: BorderRadius.circular(22),
//         border: Border.all(
//             color: _scoreColor.withValues(alpha: 0.2), width: 1),
//       ),
//       child: Row(
//         children: [
//           SizedBox(
...
//                 Row(
//                   children: [
//                     _buildScoreSegment(0.25, _safetyScore >= 25,
//                         const Color(0xFFFF2D55)),
//                     const SizedBox(width: 3),
//                     _buildScoreSegment(0.25, _safetyScore >= 50,
//                         const Color(0xFFFF9F0A)),
//                     const SizedBox(width: 3),
//                     _buildScoreSegment(0.25, _safetyScore >= 75,
//                         const Color(0xFF34C759)),
//                     const SizedBox(width: 3),
//                     _buildScoreSegment(0.25, _safetyScore >= 90, _accentBlue),
//                   ],
//                 ),
//                 const SizedBox(height: 8),
//                 Row(
...
//   Widget _buildScoreSegment(double flex, bool active, Color color) {
//     return Expanded(
...
//                 width: 3, height: 14,
//                 decoration: BoxDecoration(
//                   color: _accentBlue, borderRadius: BorderRadius.circular(2),
//                 ),
//               ),
//               const SizedBox(width: 8),
...
//         children: [
//           Icon(icon,
//               color: hasEvent ? color : const Color(0xFF3A3A4A), size: 20),
//           const SizedBox(height: 8),
//           Text('$count',
//               style: TextStyle(
...
//   // ─────────────────────────────────────────────────────────────────────────
//   //  QUICK ACTIONS  (Drive History removed)
//   // ─────────────────────────────────────────────────────────────────────────
...
//         border: Border.all(
//             color: _scoreColor.withValues(alpha: 0.3), width: 1.5),
//       ),
//       child: Column(
//         mainAxisSize: MainAxisSize.min,
//         children: [
...
//               children: [
//                 Row(children: [
//                   _statTile('Duration', _formatDuration(duration),
//                       Icons.timer_rounded, const Color(0xFF5E5CE6)),
//                   const SizedBox(width: 10),
//                   _statTile('Distance', _formatDist(distance),
...
//       final isMajor   = i % 4 == 0;
//       final outerTick = outerRadius + 6;
//       final innerTick = outerRadius + (isMajor ? 14 : 10);
//       canvas.drawLine(
//         Offset(center.dx + outerTick * cos(angle),
//             center.dy + outerTick * sin(angle)),
//         Offset(center.dx + innerTick * cos(angle),
//             center.dy + innerTick * sin(angle)),
//         tickPaint,
//       );
//     }
...
//       canvas.drawLine(
//         Offset(center.dx + (outerRadius - 5) * cos(limitAngle),
//             center.dy + (outerRadius - 5) * sin(limitAngle)),
//         Offset(center.dx + (outerRadius + 5) * cos(limitAngle),
//             center.dy + (outerRadius + 5) * sin(limitAngle)),
//         Paint()
//           ..color = const Color(0xFFFF2D55)
...
//       final nx = center.dx + outerRadius * cos(endAngle);
//       final ny = center.dy + outerRadius * sin(endAngle);
//       canvas.drawCircle(Offset(nx, ny), 6,
//           Paint()
//             ..color = color
...
//         Paint()..color = const Color(0xFF1C1C2E)..style = PaintingStyle.fill);
//     canvas.drawCircle(center, 18,
//         Paint()
...
//   @override
//   bool shouldRepaint(covariant _ProfessionalSpeedometerPainter old) =>
...
//     canvas.drawLine(
//       Offset(center.dx + (radius - 20) * cos(angle),
//           center.dy + (radius - 20) * sin(angle)),
//       Offset(center.dx + (radius + 2) * cos(angle),
...
//   final Color color;
//   final double limit;
//   const _ProfessionalSparklinePainter(
//       {required this.data, required this.color, required this.limit});
...
//     while (x < size.width) {
//       canvas.drawLine(
//         Offset(x, limitY), Offset(x + 8, limitY),
//         Paint()
//           ..color = const Color(0xFFFF2D55).withValues(alpha: 0.5)
//           ..strokeWidth = 1,
//       );
...
//   @override
//   bool shouldRepaint(covariant _ProfessionalSparklinePainter old) =>
