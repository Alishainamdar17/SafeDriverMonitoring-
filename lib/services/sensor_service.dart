...
class SensorService {
  ...
  bool isSuddenAcceleration(AccelerometerEvent event, [double? threshold]) {
    final effectiveThreshold = threshold ?? AppConfig.effectiveSuddenAccelerationThreshold;
    return event.y > effectiveThreshold;
  }

  // Calculate if there's sudden braking
  bool isSuddenBraking(AccelerometerEvent event, [double? threshold]) {
    final effectiveThreshold = threshold ?? AppConfig.effectiveSuddenBrakingThreshold;
    return event.y < effectiveThreshold;
  }

  // Calculate if there's sharp turning
  bool isSharpTurn(AccelerometerEvent event, [double? threshold]) {
    final effectiveThreshold = threshold ?? AppConfig.effectiveSharpTurnThreshold;
    return event.x.abs() > effectiveThreshold;
  }
